<template>
  <div id="swiper" @mouseover="stopAuto" @mouseout="autoPlay">
      <div class="swiper-box" :style="{width:allCount,'-webkit-transition':transitionConfig,'-webkit-transform':slateX}" ref='swiper'>
          <CarouselItem :img="imgList[imgList.length-1].img" :imgWidth="-imgWidth" v-if="loop"/>
          <CarouselItem v-for="(item,index)  in imgList" :key="index" :img="item.img" :imgWidth="imgWidth*index"></CarouselItem>
          <CarouselItem :img="imgList[0].img" :imgWidth="imgWidth*imgList.length" v-if="loop"></CarouselItem>
      </div>
