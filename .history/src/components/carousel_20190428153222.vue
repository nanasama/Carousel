<template>
  <div id="swiper" @mouseover="stopAuto" @mouseout="autoPlay">
      <div class="swiper-box" :style="{width:allCount,'-webkit-transition':transitionConfig,'-webkit-transform':slateX}" ref='swiper'>
          <swiperItem :img="imgList[imgList.length-1].img" :imgWidth="-imgWidth" v-if="loop"/>
          <swiperItem v-for="(item,index)  in imgList" :key="index" :img="item.img" :imgWidth="imgWidth*index"></swiperItem>
          <swiperItem :img="imgList[0].img" :imgWidth="imgWidth*imgList.length" v-if="loop"></swiperItem>
      </div>
