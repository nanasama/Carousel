<template>
  <div id="swiper" @mouseover="stopAuto" @mouseout="autoPlay">
      <div class="swiper-box" :style="{width:allCount,'-webkit-transition':transitionConfig,'-webkit-transform':slateX}" ref='swiper'>
          <swiper-item :img="imgList[imgList.length-1].img" :imgWidth="-imgWidth" v-if="loop"></swiper-item>
          <!-- <swiper-item v-for="(item,index) in imgList" :img="item.img" :imgWidth="imgWidth*index"></swiper-item> -->
          <swiper-item :img="imgList[0].img" :imgWidth="imgWidth*imgList.length" v-if="loop"></swiper-item>
      </div>
